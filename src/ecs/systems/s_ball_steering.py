...
def system_ball_steering(world:esper.World, ball_cfg:dict, delta_time:float):
    ...
    for _, (c_t, c_v, c_bs, _) in components:
        ...
        # Follow Vector
        follow_dampening = good_block_dist / 100
        c_bs.follow_vector = closest_good - c_t.pos
        desired_good_length = min(ball_cfg["follow_force"], ball_cfg["follow_force"] / follow_dampening)
...
def _get_closest_good_block_pos(world:esper.World, ball_pos:pygame.Vector2) -> pygame.Vector2:
    ...
    min_dist = None
    components = world.get_components(CTransform, CTagBlock)
    for _, (c_t, c_bl) in components:
        if c_bl.b_type == "B1":
